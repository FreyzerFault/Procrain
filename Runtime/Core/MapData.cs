...
namespace Procrain.Core
{
    ...
    public class MapData: AutoUpdatableSo
    {
        ...
        public Mesh Mesh => MeshData.CreateMesh();
        public Mesh ApplyToMesh(Mesh mesh) => MeshData.ApplyMesh(mesh);
        
        public Mesh MeshByLOD(int lodIndex) => MeshDataByLOD(lodIndex).CreateMesh();
        public Mesh ApplyToMeshByLOD(Mesh mesh, int lodIndex) => MeshDataByLOD(lodIndex).ApplyMesh(mesh);
        public Mesh ApplyToMeshByLOD_Pow2(Mesh mesh, int lod) => MeshDataByLOD_Pow2(lod).ApplyMesh(mesh);

        
        #region EVENTS

        public bool autoUpdateOnParameterChanges = true;
        [FormerlySerializedAs("subscribed")] [FormerlySerializedAs("suscribed")] public bool isSubscribed = false;
        ...
        public IEnumerator BuildAllDataCoroutine()
        {
            ...
            yield return heightMapBuilder?.BuildCoroutine(heightMapParams, onEnd: hm => heightMap = hm);
            OnHeightMapUpdated?.Invoke(heightMap);
