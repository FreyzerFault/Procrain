...
namespace Procrain.MapBuilder
{
    ...
    public class HeightMapBuilder_PerlinNoise : HeightMapBuilder
    {
        ...
        public override IHeightMap Build(IHeightMapParams perlinNoiseParams)
        {
            if (perlinNoiseParams is PerlinNoiseParams np)
            {
                float[] map = PerlinNoise.BuildHeightMap(np, np.HeightCurve != null ? np.HeightCurve.Evaluate : null);
                return new HeightMap(map, np.Size + 1, np.Seed);
            }

            throw new ArgumentException("Los parámetros deben ser de tipo PerlinNoiseParams", nameof(perlinNoiseParams));
        }
            ...
            IHeightMapParams heightParams, Action onStart = null, Action<IHeightMap> onEnd = null)
        {
            ...
            if (paralelized)
                yield return BuildHeightMap_ParallelizedCoroutine(np.ToThreadSafe(), onStart, onEnd);
            else
            {
                onStart?.Invoke();
                float[] map = PerlinNoise.BuildHeightMap(np, np.HeightCurve != null ? np.HeightCurve.Evaluate : null);
        ...
        {
            if (parameters is not PerlinNoiseParams_ThreadSafe np) yield break;
            
            float time = Time.time;

            int sampleSize = np.SampleSize;
            uint seed = np.seed;
