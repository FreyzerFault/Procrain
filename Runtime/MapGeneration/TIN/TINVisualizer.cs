...
namespace Procrain.MapGeneration.TIN
{
	...
	public class TinVisualizer : MonoBehaviour
	{
		...
		private void UpdateLineExtremes(Vector2 point, PointerEventData.InputButton button)
		{
			...
			GameObject sprite = minimap.DrawPointInMousePosition(leftClick ? Color.red : Color.green);
			...
			if (leftClick)
			{
				...
				_startPoint2D = point;
				if (tin.GetHeightInterpolated(point, out float height))
		...
		private Vector2 GetMousePoint2D(Vector3 mousePoint)
		{
			...
			Vector2 screenPoint = minimap.GetScreenSpaceMousePoint(mousePoint);
		...
		private void OnDrawGizmosPath()
		{
			...
			foreach (Vector3 intersection in intersections) Gizmos.DrawSphere(intersection, 1);
		}
		...
		private void InitializeTin()
		{
			...
			tin = new Tin(heightMap.map, heightMap.Size, errorTolerance, HeightScale);
