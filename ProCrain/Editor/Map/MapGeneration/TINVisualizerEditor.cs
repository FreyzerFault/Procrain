    ...
    internal class TinVisualizerEditor : UnityEditor.Editor
    {
        ...
        public void OnSceneGUI()
        {
            ...
            if (tin.lastVertexAdded is { Count: > 0 })
                ...
                {
                    Handles.Label(tin.lastVertexAdded[i], tin.lastVertexAdded[i].ToString(), cyanStyle);
                    Handles.Label(
                        tin.lastVertexAdded[i] + Vector3.up * 10,
            ...
            if (showHeightInfo)
                foreach (var v in tin.vertices)
                {
                    ...
                    Handles.Label(v.xyz + (float3)Vector3.up * 10, v.y.ToString(CultureInfo.InvariantCulture), cyanStyle);
                }

            if (showVertexIndices || showEdgeInfo)
                foreach (var e in tin.edges)
                {
                    if (showEdgeInfo) Handles.Label((e.end.xyz + e.begin.xyz) / 2, e.ToString(), magentaStyle);

                    if (!showVertexIndices) continue;

                    Handles.Label(e.begin.xyz, e.begin.ToString(), greenStyle);
                    Handles.Label(e.end.xyz, e.end.ToString(), greenStyle);
                }
