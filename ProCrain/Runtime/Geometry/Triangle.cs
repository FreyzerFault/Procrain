...
namespace Procrain
{
    public struct Triangle: IEquatable<Triangle>
	{
		...
		public Edge[] EdgeArray => new[] { e1, e2, e3 };
		...
		public static Triangle InvalidTri => new(Vertex.InvalidVertex, Vertex.InvalidVertex, Vertex.InvalidVertex); 
		...
		public int GetOppositeTriangle(Edge edge)
		{
			if (edge.Equals(e1)) return e1.OppositeTri(index);
			if (edge.Equals(e2)) return e2.OppositeTri(index);
			if (edge.Equals(e3)) return e3.OppositeTri(index);
		...
		public bool GetIntersectionPoint(
			Vector2 a, Vector2 b, out Vector2? intersectionPoint, out int nextTriangle
		)
		{
			...
			if (e1.GetIntersectionPoint(a, b, out intersectionPoint))
			{
				...
				nextTriangle = e1.OppositeTri(index);
				return true;
			}
			...
			{
				nextTriangle = e2.OppositeTri(index);
				return true;
			}
			...
			{
				nextTriangle = e3.OppositeTri(index);
				return true;
			}
		...
		public bool GetIntersectionPoint(
			...
			out int nextTriangle, out Vector2? nextPoint
		)
		{
			...
			if (Intersect(a, b))
			{
				...
				if (farEdge.IsInvalid) nextTriangle = farEdge.OppositeTri(index);
