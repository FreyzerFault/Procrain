...
namespace Procrain
{
    ...
    public class HeightMap : IHeightMap
    {
        ...
        public static HeightMap CreatePerlinNoiseHeightMap(
            PerlinNoiseParams_ThreadSafe np,
        ...
        public static HeightMap CreatePerlinNoiseHeightMap(PerlinNoiseParams np, AnimationCurve heightCurve = null) =>
            CreatePerlinNoiseHeightMap(np.ToThreadSafe(), heightCurve);
    }
    ...
    public struct HeightMap_ThreadSafe : IHeightMap, IDisposable
    {
        ...
        public HeightMap_ThreadSafe(PerlinNoiseParams_ThreadSafe noiseParams)
            : this(noiseParams.size, noiseParams.seed) {}
        public HeightMap_ThreadSafe(PerlinNoiseParams noiseParams)
            : this(noiseParams.ToThreadSafe()) {}
            
        public HeightMap_ThreadSafe(float[] heightMap, int size = 129, uint seed = 0)
        {
            this.size = size;
            this.seed = seed;

            map = new NativeArray<float>(heightMap, Allocator.Persistent);
        }

        public HeightMap_ThreadSafe(float[,] heightMap, int size = 129, uint seed = 0)
            : this(heightMap.Flatten(), size, seed) {}
        
        public HeightMap_ThreadSafe(UnityEngine.Terrain terrain, uint seed = 0)
            : this(
                FlipCoordsXY(
                    terrain.terrainData.GetHeights(
                        0,
        ...
        [BurstCompile]
        public float GetHeight(int2 coord) => map[coord.x + coord.y * Size];
        
        public float GetHeight(Vector2Int coord) => map[coord.x + coord.y * Size];
        ...
        private void NormalizeToMaxHeight()
        {
            float maxHeight = map.Max();
            float minHeight = map.Min();
            for (int i = 0; i < map.Length; i++)
                map[i] = Mathf.InverseLerp(minHeight, maxHeight, map[i]);
        }
        
        public static HeightMap_ThreadSafe CreatePerlinNoiseHeightMap(
            PerlinNoiseParams_ThreadSafe np,
            AnimationCurve heightCurve = null
        )
        {
            HeightMap_ThreadSafe heigthMap = new(PerlinNoise.BuildHeightMap(np), np.Size + 1, np.seed);
        ...
        public static float[,] FlipCoordsXY(float[,] map)
        {
            float[,] map2DFlipped = new float[map.GetLength(1), map.GetLength(0)];
