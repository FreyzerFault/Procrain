...
namespace Procrain
{
	...
	[RequireComponent(typeof(UnityEngine.Terrain), typeof(TerrainCollider))]
	public class MapDisplayInTerrain : MapDisplayBase
	{
		...
		private UnityEngine.Terrain Terrain => _terrain ? _terrain : GetComponent<UnityEngine.Terrain>();
		...
		private void UpdateTerrainData(IHeightMap heightMap)
		{
			...
			terrainData.ApplyToHeightMap(
				heightMap,
				mapData.terrainParams.HeightScale,
				resolutionAmplifier
			);
		...
		private void Update()
		{
			if (!movement)
				return;
			mapData.heightMapParams.Offset += new float2(1, 0);
		}
