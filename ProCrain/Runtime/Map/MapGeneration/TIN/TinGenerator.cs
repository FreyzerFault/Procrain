using System;
using Procrain.Mesh;
using Unity.Mathematics;
using UnityEngine;

namespace Procrain.TIN
{
	public static class TinGenerator
	{
		/// <summary>
		///     Genera un TIN a partir de un HeightMap
		/// </summary>
		/// <param name="heightMap">Mapa de Alturas</param>
		/// <param name="bounds">Bounding Box</param>
		/// <param name="heigthScale">[0,1] -> [0, heightScale]</param>
		/// <param name="errorTolerance">Error Maximo Tolerado por el Tin</param>
		/// <param name="maxIterations">Limite de iteraciones maximas del Tin</param>
		/// <returns>Datos de una Malla que va a usar Unity</returns>
		private static Tin BuildTin(
			HeightMap_ThreadSafe heightMap, float heigthScale = 1, float errorTolerance = 1,
			int maxIterations = 10, AABB_2D? bounds = null
		)
		{
			// Creacion del Tin (Estructura topologica interna)
			Tin tin = new(heightMap.map, heightMap.size, bounds, errorTolerance, heigthScale, maxIterations);
			tin.InitGeometry();
			tin.AddPointLoop();

			// Creacion de la Malla
			return tin;
		}

		/// <summary>
		///     Genera un TIN a partir de unos puntos iniciales (las esquinas formadas por el AABB)
		/// </summary>
		/// <param name="aabb">Bounding Box</param>
		/// <param name="errorTolerance">Error Maximo Tolerado por el Tin</param>
		/// <param name="heightScale"></param>
		/// <param name="maxIterations">Limite de iteraciones maximas del Tin</param>
		/// <param name="points">Puntos extras pregenerados</param>
		/// <returns>Datos de una Malla que va a usar Unity</returns>
		private static Tin BuildTin(
			AABB_2D aabb, float errorTolerance = 1, float heightScale = 100,
			int maxIterations = 10, Vector3[] points = null
		)
		{
			// Creacion del Tin (Estructura topologica interna)
			var tin = new Tin(points ?? Array.Empty<Vector3>(), errorTolerance, heightScale, maxIterations, aabb);
			tin.InitGeometry();
			tin.AddPointLoop();

			// Creacion de la Malla
			return tin;
		}

		/// <summary>
		///     Generacion de la Malla a partir de un Tin
		/// </summary>
		/// <param name="tin"></param>
		/// <returns>Datos de una Malla que va a usar en Unity</returns>
		public static MeshDataDynamic TinToMesh(Tin tin)
		{
			// Creacion de la malla (Datos basicos que necesita Unity)
			MeshDataDynamic data = new(tin.vertices.Count, tin.triangles.Count);

			for (int i = 0; i < tin.triangles.Count; i++)
			{
				for (int v = 0; v < 3; v++)
				{
					Vertex vertex = tin.triangles[i].Vertices[v];
					data.AddVertex(vertex.xyz, new float2(vertex.x / tin.mapSize, vertex.z / tin.mapSize));
				}

				data.AddTriangle(i * 3 + 0, i * 3 + 1, i * 3 + 2, true);
			}

			return data;
		}

		public static IMeshData BuildTinMeshData(
			out Tin tin, HeightMap_ThreadSafe heightMap, float errorTolerance = 1,
			float heightScale = 100,
			int maxIterations = 10
		)
		{
			tin = BuildTin(heightMap, heightScale, errorTolerance, maxIterations);
			return TinToMesh(tin);
		}

		public static IMeshData BuildTinMeshData(
			out Tin tin, HeightMap heightMap, float errorTolerance = 1,
			float heightScale = 100,
			int maxIterations = 10
		) => BuildTinMeshData(out tin, heightMap.ToThreadSafe(), errorTolerance, heightScale, maxIterations);
	}
}
