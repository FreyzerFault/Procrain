...
namespace Procrain.Mesh
{
	public static class MeshGenerator
	{
		...
		)
		{
			...
			PerlinNoise_ThreadSafe.PerlinOctaves octaves = new(noiseParams);
			
			// Scale no puede ser negativa
			float scale = noiseParams.scale > 0 ? noiseParams.scale : 0.0001f;

			int size = noiseParams.SampleSize;
			float heightScale = terrainParams.HeightScale;

			float halfSize = size / 2f;
			float2 center = new float2(halfSize, halfSize) + noiseParams.Offset;
			...
			for (var x = 0; x < size; x += simplificationIncrement)
			{
				...
				if (!noiseParams.heightCurve.IsEmpty) 
					heightValue = noiseParams.heightCurve.Evaluate(heightValue);
		...
		public static IMeshData BuildMeshData(PerlinNoiseParams noiseParams, ITerrainParams terrainParams,
			Gradient gradient = null) => BuildMeshData(noiseParams.ToThreadSafe(), terrainParams, gradient);
	}
