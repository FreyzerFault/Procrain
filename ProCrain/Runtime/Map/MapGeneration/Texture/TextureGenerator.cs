...
using UnityEngine;
using static Procrain.PerlinNoise_ThreadSafe;
using Octaves = Procrain.PerlinNoise.PerlinOctaves;

namespace Procrain.Texture
{
    public static class TextureGenerator
    {
        #region TEXTURE DATA
                ...
                , Func<Color, Color> postprocessing = null)
            {
                ...
                for (int y = 0; y < textureParams.Height; y++)
                for (int x = 0; x < textureParams.Width; x++)
                {
                    ...
                    float interpolatedHeight = textureParams.Filter switch
                    {
                        ITextureParams.InterpolationFilter.Nearest => 
                            heightMap.GetHeight(Vector2Int.FloorToInt(sourceCoords)),
            ...
            {
                return texParams switch
                {
                    TextureParams tp => BuildTextureData(noiseParams, tp),
                    TextureParams_ThreadSafe tp => BuildTextureData(noiseParams, tp),
                    _ => throw new ArgumentException(
                        "NoiseParams and TextureParams must be of types " +
                        "PerlinNoiseParams_ThreadSafe " +
            ...
            {
                Vector2Int res = new(texParams.Width, texParams.Height);
                PerlinOctaves octaves = new(noiseParams);
                ...
                for (var x = 0; x < res.x; x++)
                {
                    ...
                    float value = PerlinNoise_ThreadSafe.GetNoiseHeight(coords, noiseParams.scale, octaves);
        ...
        #region TEXTURE2D
            ...
            public static Texture2D BuildTexture2D(HeightMap_ThreadSafe map, ITextureParams texParams) =>
                ColorDataToTexture2D(BuildTextureData(map, texParams), texParams.Width, texParams.Height);
                
            public static Texture2D BuildTexture2D(HeightMap_ThreadSafe map, Gradient gradient) =>
            ...
            public static Texture2D BuildTexture2D(HeightMap map, Gradient gradient) =>
                BuildTexture2D(map.ToThreadSafe(), gradient);
            public static Texture2D BuildTexture2D(HeightMap map, ITextureParams texParams) =>
                BuildTexture2D(map.ToThreadSafe(), texParams);
            ...
            public static Texture2D BuildTexture2D(PerlinNoiseParams_ThreadSafe noiseParams, ITextureParams texParams) =>
        ...
        public struct GeneratePerlinNoiseTextureJob : IJob
        {
            ...
            public GeneratePerlinNoiseTextureJob(PerlinNoiseParams_ThreadSafe noiseParams,
