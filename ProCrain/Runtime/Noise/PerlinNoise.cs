...
namespace Procrain
{
	public static class PerlinNoise
	{
		...
		public static float[] BuildHeightMap(PerlinNoiseParams_ThreadSafe np, Func<float, float> valuePostProcessor = null)
		{
		...
		public static float[] BuildHeightMap(PerlinNoiseParams np, Func<float, float> valuePostProcessor = null) =>
			BuildHeightMap(np.ToThreadSafe(), valuePostProcessor);
		...
		/// <returns>Flat 1D Map</returns>
		public static T[] BuildMap<T>(PerlinNoiseParams_ThreadSafe np, Func<float, T> valuePostProcessor)
		{
			PerlinOctaves octaves = new(np);
		...
		public static T[] BuildMap<T>(PerlinNoiseParams np, Func<float, T> valuePostProcessor) =>
			BuildMap(np.ToThreadSafe(), valuePostProcessor);
		...
		public static float GetNoiseHeight(float2 point, PerlinNoiseParams_ThreadSafe np, PerlinOctaves octaves) =>
		...
		public struct PerlinOctaves
		{
			...
			public PerlinOctaves(PerlinNoiseParams_ThreadSafe np) : this(np.numOctaves)
			{
		...
		public static float3[] SampleNoiseInPointsFromFile(PerlinNoiseParams_ThreadSafe np, string filePath, out AABB_2D aabb)
		{
			PerlinOctaves octaves = new(np);
		...
		private static float3[] GetWorldCorners(AABB_2D aabb, PerlinNoiseParams_ThreadSafe np, PerlinOctaves octaves) => new[]
		{
	...
	public static class PerlinNoise_ThreadSafe
	{
		...
		public static void BuildHeightMap(NativeArray<float> map, PerlinNoiseParams_ThreadSafe np) =>
			BuildHeightMap(map, np, new PerlinOctaves(np));

		[BurstCompile]
		public static void BuildHeightMap(
			NativeArray<float> map, PerlinNoiseParams_ThreadSafe np, PerlinOctaves octaves
		)
		{
		...
		public static float GetNoiseHeight(
			float2 point, float scale, PerlinOctaves octaves
		)
		{
			float height = 0;
			for (var i = 0; i < octaves.Count; i++)
		...
		public readonly struct PerlinOctaves
		{
