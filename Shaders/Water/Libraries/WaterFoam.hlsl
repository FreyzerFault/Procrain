...
float2 DistortUV(float2 UV, float Amount)
{
    ...
    UV.y += Amount * 0.01 * (sin(UV.x * 3.5 + time * 0.35) + sin(UV.x * 4.8 + time * 1.05) + sin(
        UV.x * 7.3 + time * 0.45)) / 3.0;
    UV.x += Amount * 0.12 * (sin(UV.y * 4.0 + time * 0.50) + sin(UV.y * 6.8 + time * 0.75) + sin(
        UV.y * 11.3 + time * 0.2)) / 3.0;
    UV.y += Amount * 0.12 * (sin(UV.x * 4.2 + time * 0.64) + sin(UV.x * 6.3 + time * 1.65) + sin(
        UV.x * 8.2 + time * 0.45)) / 3.0;

    return UV;
}

float4 SurfaceFoam(float2 uv, float2 movement, float2 offset, float scale, float3 sampling, SamplerState Sampler,
                   Texture2D Texture)
{
...
void SurfaceFoam_float(float2 uv, float4 movement, float2 scale, float2 offset, float3 sampling, SamplerState Sampler,
                       Texture2D Texture, out float Primary, out float Secondary)
{
...
void SampleSurfaceFoamTexture_half(half4 uvs, half2 sampling, SamplerState Sampler, Texture2D tex, out half Foam1,
                                   out half Foam2)
{
...
void SampleSurfaceFoamTextureShadows_half(half4 uvs, half2 sampling, SamplerState Sampler, Texture2D tex, half2 offset,
                                          out half Shadow1, out half Shadow2)
{
...
void FoamColor_half(half4 foamcolor1, half4 foamcolor2, half foam1, half foam2, half shadow1, half shadow2,
                    out half Shadow, out half4 Foam)
{
...
void FoamUV_float(float2 uv, float4 movement, float2 offset, float2 scale, float depth, out float4 ByDepth,
                  out float4 Directional)
{
...
void SampleTexture_float(float4 uvs, float distortion, SamplerState Sampler, Texture2D Texture, out float R,
                         out float G)
{
