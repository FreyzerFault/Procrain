        ...
        public void OnSceneGUI()
        {
            ...
            if (showHeightInfo)
                foreach (var v in tin.vertices)
                {
                    ...
                    Handles.Label(v + Vector3.up * 10, v.y.ToString(CultureInfo.InvariantCulture), cyanStyle);
                }

            if (showVertexIndices || showEdgeInfo)
                foreach (var e in tin.edges)
                {
                    if (showEdgeInfo) Handles.Label((e.end + e.begin) / 2, e.ToString(), magentaStyle);

                    if (!showVertexIndices) continue;

                    Handles.Label(e.begin, e.begin.ToString(), greenStyle);
                    Handles.Label(e.end, e.end.ToString(), greenStyle);
                }
